...
#' source data, x_{ij}
tableSource <- freqtable %>% select(Other:Ruminants)
...
humanCov <- humantypes %>% select(ST, UR_num) %>%
    rename("Rurality"=UR_num) %>% na.omit
...
sampDistST <- sampDist %>% select(Poultry, Other, Ruminants) %>%
    rename("sampS1"=Poultry, "sampS2"=Other, "sampS3"=Ruminants)
