...
############################################# choose baseline
#' basline: Ruminants
freqtable <- freqtable %>% dplyr::select(Human, Poultry, Water,
                                         Other, Ruminants)
...
load(file="OFbayes_wCovH2_baseline.RData")
...
postt_all <- as.tibble(rbind(post_beta0, post_beta1)) %>% 
    mutate(source=factor(recode(source, "V1"="Poultry",
                                "V2"="Water",
                                "V3"="Other", "V4"="Poultry",
                                "V5"="Water", "V6"="Other"),
                         levels=c("Water", "Poultry", "Other")),
...
newbeta0 <- postt_all %>% dplyr::select(-para_level) %>%
    ...
    dplyr::select(Poultry, Water, Other)
...
pdf(file="fig_ch3_appxC11_C12_C13.pdf", height=7, width=7)
