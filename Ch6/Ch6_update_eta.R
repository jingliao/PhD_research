...
update_eta <- function(data, state, prior) {
  ...
  for (source in source_order) {
    ...
    for (type in type_order) {
      ...
      if (nrow(data$H) > 0) {
        ...
        pi_h_star[,source] <- compute_pi_j(eta=eta_star*exp(state$c[,source]))
      ...
      logalpha <- ratio_lh + ratio_propE + ratio_priorE
 ...
 state$accept_eta <- state$accept_eta + c(accept, reject)
